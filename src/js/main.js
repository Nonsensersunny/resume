    ...
    load() {
        return this.getUserObjectID().then((userID) => {
            ...
            return Promise.all([contact, basicInfo, workExperience, personalProjects, skills, settings]).then((val) => {
                ...
                let splittedProjects = val[3].split();
                this.personalProjects.projects = splittedProjects.per_projects;
                this.workExperience.experiences.forEach((item, idx) => {
                    item.projects = splittedProjects.exp_projects[item.workExperienceID];
                });
...
class Experience {
    ...
    constructor(workExperienceID, name, logo, date, projects=[]) {
...
class PersonalProjects {
    ...
    split() {
        ...
        this.projects.forEach((pro, idx) => {
            if (pro.isPersonal) {
                splittedProjects.per_projects.push(pro);
            } else {
                let cur = splittedProjects.exp_projects[pro.experienceID];
                if (cur === undefined) {
                    splittedProjects.exp_projects[pro.experienceID] = [pro];
                } else {
                    splittedProjects.exp_projects[pro.experienceID].push(pro);
                }
            }
        })
        return splittedProjects;
    }
}
...
class Project {
    ...
    constructor(id, userID, isPersonal, expID, name, intro, techs, thumb, fame, source, address, github) {
        this.id = id;
        this.userID = userID;
    ...
    static loadByExperienceID(experienceID) {
        let query = new AV.Query(this.tableName());
        query.equalTo("experience_id", experienceID);
        return query.find().then((ps) => {
            let projects = [];
            ps.forEach((p, idx) => {
